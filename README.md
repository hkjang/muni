...
| --- | --- | --- | --- |
| HWP | O | - | 한글 오피스의 옛 이진 형식입니다. OLE2 복합 파일 안에 압축된 이진 레코드가 들어 있습니다. Import 는 글자와 문단, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 문단 정렬·들여쓰기·줄간격, 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 표(가로·세로 병합, 열 너비, 칸 음영·세로 정렬)와 표 캡션, 그림(그려진 크기대로, 옛 파일의 BMP 는 PNG 로 바꿔서), 글상자와 묶음 개체 안의 글, 각주·미주, 머리말·꼬리말, 쪽 나누기, 용지 방향을 읽습니다. 암호가 걸린 문서는 무엇이 문제인지 말하고 거절합니다. |
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 표(가로·세로 병합, 열 너비, 머리글 행, 칸 음영·세로 정렬), 그림(그려진 크기대로), 각주, 머리말·꼬리말, 쪽 나누기, 용지 방향을 양방향으로 처리합니다. 칸 음영은 칸이 아니라 머리의 `borderFill` 에 있어서, 칸만 보던 동안에는 한글이 쓴 어떤 파일에서도 읽히지 않았습니다. 쪽 나누기는 나뉜 자리가 아니라 *다음* 문단이 지니고 있어서 — `.hwp` 는 문단 나누기 종류의 셋째 비트, `.hwpx` 는 `<hp:p pageBreak="1">` — 그 문단 앞에 놓아야 합니다. 쓰는 파일은 부품과 요소 하나하나가 한글 자신이 쓰는 모양입니다. |
...
| Mermaid | - | O | 슬래시 메뉴의 「도형 (mermaid)」으로 넣거나 `mermaid` 언어를 지정한 코드 블록을 쓰면 편집기에서 도형으로 그려지고, HTML·PDF 로 내보낼 때도 그림으로 나갑니다. 다른 형식에서는 도형을 담을 수 없으므로 코드 그대로 남습니다 — 도형이 말하는 바가 곧 그 글이기 때문입니다. |
...
`.hwp` 는 세 겹을 벗겨야 글자가 나옵니다 — OLE2 복합 파일, 스트림마다 걸린 raw deflate 압축(zlib 껍데기가 없어서 zlib 리더는 "invalid header" 를 냅니다), 그리고 레코드입니다. 레코드는 각자 깊이를 지니고 한 줄로 늘어서 있어서, 그것을 트리로 되돌려야 무엇이 무엇에 속하는지 알 수 있습니다 — 문단의 글자와 서식, 표의 칸, 칸 안의 문단이 모두 그렇습니다. 문단 글자 안의 제어 표시는 **크기가 8 WCHAR, 즉 16바이트**입니다 — 형식 문서의 "8" 을 바이트로 읽으면 표시 한가운데에 내려앉아 그 뒷부분을 글자로 읽고, 문서에 없던 한자가 흩뿌려집니다. **서식이 가리키는 위치도 그 8을 세어야 합니다** — 살아남은 글자만 세면 첫 표나 그림 뒤의 서식이 전부 엉뚱한 낱말에 붙습니다. 그리고 **표의 칸·글상자·각주를 뜻하는 LIST_HEADER 는 자기 문단들 *위*가 아니라 *앞*에, 같은 깊이로 놓입니다** — 깊이만 보고 트리를 만들면 모든 칸이 비어 보이고 그 글은 표 옆에 매달립니다. 이것은 실제 파일 열일곱 개를 읽고서야 알았고, 그 전의 시험 파일은 읽는 쪽이 바라는 대로 한 단계 깊게 넣어 두고 있었습니다. 글자의 서식도 마찬가지로 그대로 읽으면 안 됩니다 — **색은 COLORREF, 즉 0x00BBGGRR 이라 파랑이 먼저이고**, RGB 로 읽으면 붉게 강조한 낱말이 전부 파랗게 들어옵니다. **글꼴은 이름이 아니라 FACE_NAME 목록의 번호로 적히며**, 그 목록은 한글 글꼴부터 차례로 놓이므로 한글 글꼴 번호는 목록의 앞에서부터 세면 됩니다.

**두 한글 리더는 공개된 실제 파일로 확인했습니다** — pyhwp·hwp.js·rhwp 가 공개한 `.hwp` 열일곱 개와 `.hwpx` 열다섯 개(정부 결재 문서, 보도자료, 별지 서식 포함). 전부 읽히고, `.hwp` 열일곱 개는 `.hwpx` 로 써서 다시 읽어도 글자와 구조가 같습니다. **그 왕복은 muni 의 리더가 muni 의 리더를 검사하는 것이라**, 쓴 파일은 다른 구현(rhwp)으로도 읽어 원본 `.hwp` 의 글자와 대조했습니다 — 열일곱 중 열둘이 글자까지 같고, 나머지는 각주·개요 *번호* 를 글자로 그리는 차이뿐입니다(단추·확인란의 이름 두 줄은 문단이 아니라 서식 개체라 들어오지 않습니다). 그 대조가 리더의 결함 셋(글꼴 번호를 이름으로 읽던 것, 칸의 세로 정렬을 엉뚱한 요소에서 찾던 것, `switch` 안의 여백을 못 보던 것)과 빠진 것 셋(목록, 머리말·꼬리말, 표 캡션)을 찾아냈습니다. `MUNI_CORPUS=<dir> go test ./internal/hwp/ ./internal/hwpx/ -run 'Corpus|RealRoundTrip' -v` 로 아무 폴더의 파일이든 같은 검사를 돌릴 수 있고, `MUNI_OUT=<dir>` 을 더하면 쓴 `.hwpx` 를 남겨 한글이나 다른 리더로 열어 볼 수 있습니다.

**HWPX 는 서식을 문단이나 런에 적지 않고 머리(`Contents/header.xml`)에 적어 두고 번호로 가리킵니다.** 런에는 `charPrIDRef="7"` 하나만 있어서, 런만 읽으면 서식이 아예 없습니다 — muni 가 워드 스타일을 잃던 것과 같은 모양이라 이 가져오기는 머리부터 읽습니다. 표는 한글에서 문단 안에 들어 있고 muni 에서는 블록이므로 밖으로 꺼내며, 그림도 같습니다. **Export 는 Import 의 시험입니다** — muni 가 `.hwpx` 를 쓰지 못하던 동안 읽기 테스트는 전부 손으로 만든 파일이었고, 쓰기가 생기자 첫 왕복에서 읽기의 결함 둘(각주를 읽지 않던 것, 첫 줄 들여쓰기의 철자)이 바로 드러났습니다.
