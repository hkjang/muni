...
| --- | --- | --- | --- |
| HWP | O | - | 한글 오피스의 옛 이진 형식입니다. OLE2 복합 파일 안에 압축된 이진 레코드가 들어 있습니다. Import 는 글자와 문단, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 문단 정렬·들여쓰기·줄간격, 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 표(가로·세로 병합, 열 너비, 칸 음영·세로 정렬)와 표 캡션, 그림(그려진 크기대로, 옛 파일의 BMP 는 PNG 로 바꿔서), 글상자와 묶음 개체 안의 글, 각주·미주, 머리말·꼬리말, 쪽 나누기, 구분선, 용지 방향을 읽습니다. 암호가 걸린 문서는 무엇이 문제인지 말하고 거절합니다. |
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 표(가로·세로 병합, 열 너비, 머리글 행, 칸 음영·세로 정렬), 그림(그려진 크기대로), 각주, 머리말·꼬리말, 쪽 나누기, 인용문·코드 블록, 구분선, 용지 방향을 양방향으로 처리합니다. 한글에는 인용문도 코드 블록도 없어서 muni 가 머리에 스타일 둘(`인용`/`Quote`, `코드`/`Code`)을 심어 두고 문단이 그것을 가리킵니다 — 들여쓰고 고정폭으로 그린 문단은 읽는 쪽에 그저 문단이라, 이름 말고는 되살릴 단서가 없습니다. 코드 블록은 한 문단 안에 줄바꿈으로 넣습니다. 줄마다 문단이면 끝을 알릴 것이 없어 나란한 코드 블록 둘이 하나로 돌아옵니다. 칸 음영은 칸이 아니라 머리의 `borderFill` 에 있어서, 칸만 보던 동안에는 한글이 쓴 어떤 파일에서도 읽히지 않았습니다. 쪽 나누기는 나뉜 자리가 아니라 *다음* 문단이 지니고 있어서 — `.hwp` 는 문단 나누기 종류의 셋째 비트, `.hwpx` 는 `<hp:p pageBreak="1">` — 그 문단 앞에 놓아야 합니다. 구분선도 한글에는 없습니다 — 한글이 하이픈 한 줄을 그리는 방법이 곧 muni 가 쓰는 방법으로, 아무것도 담지 않은 문단 아래에 선 하나를 긋고 그 테두리를 머리의 `borderFill` 로 가리킵니다. 네 면에 두른 선은 글을 감싼 상자이지 구분선이 아니므로, 위나 아래에만 그은 것을 읽습니다. 그전에는 `─` 서른 개를 글자로 썼고, 글자는 글자로 돌아와 두 번 오간 문서에는 선이 있던 자리에 그 문단이 남았습니다. 쓰는 파일은 부품과 요소 하나하나가 한글 자신이 쓰는 모양입니다. |
| DOCX | O | O | 제목 단계, 글머리·번호·체크 목록(중첩 포함), 표(가로·세로 병합, 열 너비, 머리글 행, 칸 음영·세로 정렬), 굵게·기울임·밑줄·취소선·코드·형광·글자색·글꼴·크기·위첨자·아래첨자, 하이퍼링크, 문단 정렬·들여쓰기·줄간격, 인용, 코드 블록, 구분선, 쪽 나누기, 이미지, 각주·미주, 목차, 머리글·바닥글, 용지 방향을 처리하고, Word 가 파일을 끼워 넣을 때 남기는 `altChunk` 도 따라 읽습니다. |
| PDF | O | O | Import는 텍스트 레이어에서 제목·문단·목록·표·코드 블록·이미지와 하이퍼링크를 복원합니다. 굵기·기울임·첨자는 줄이 아니라 그려진 조각마다 붙습니다 — 각주 번호와 ㎡ 의 2 는 작게, 줄에서 살짝 올려 그린 글자입니다. 합자(ﬁ)는 글자로 풀고, 낱말 간격은 형식이 정한 대로 한 바이트 빈칸에만 더합니다. 쪽마다 되풀이되는 머리말·꼬리말과 쪽 번호는 종이의 것이지 문서의 것이 아니므로 버리고, 쪽을 넘어 이어지는 표는 하나로 잇습니다. 글자는 내장 글꼴의 자체 문자표로 확인합니다 — 부분 내장 글꼴에 딸린 `/ToUnicode` 는 글리프 번호를 그대로 유니코드라고 적어 두는 일이 잦아서, 그것만 믿으면 띄어쓰기가 `$` 나 제어 문자로 나옵니다. Export는 Chromium 렌더링으로 화면과 같은 레이아웃을 만듭니다. |
...
**목록 화면에 파일을 끌어다 놓으면 새 문서로 가져옵니다.** 홈과 워크스페이스 어디에나 놓을 수 있고, 가져오기 창이 파일을 든 채로 열려 워크스페이스·폴더·제목만 확인하면 됩니다. 워크스페이스에서는 보고 있는 폴더가 이미 골라져 있습니다. 여러 개를 놓으면 파일마다 문서 하나씩 차례로 만듭니다.

**열린 문서에 파일을 끌어다 놓으면 그 자리에 내용이 들어갑니다.** 편집 중인 문서에 PDF·DOCX·HWP·HWPX·Markdown·TXT·HTML 을 놓으면 서버가 같은 파서로 읽어 돌려주고, 편집기가 놓인 자리 — 문단 위면 그 문단 뒤, 여백이면 문서 끝 — 에 한 번의 편집으로 넣어 실행 취소 한 번에 되돌아갑니다. 그림은 이미지로 들어가고, 여러 파일은 놓은 차례대로 하나씩 들어가며, 읽지 못한 파일은 나머지를 막지 않고 알립니다. 빈 문서에 놓으면 파일이 그 문서가 됩니다 — 제목·머리말·꼬리말·용지 방향까지. 클립보드의 파일을 붙여 넣어도, `/파일` 로 골라도 같은 길입니다. 문서 밖에 떨어뜨려도 브라우저가 파일을 열며 편집 중인 문서를 떠나는 일은 없습니다.

**가져오기는 남이 만든 파일을 읽는 일입니다.** 그래서 파일이 말하는 것을 곧이곧대로 믿지 않습니다. 압축 파일은 풀기 전에 **디렉터리가 말하는 부품 수와 푼 크기의 합**을 봅니다 — deflate 는 천 배까지 부풀 수 있어서 작은 파일이 큰 파일이 아니라는 보장이 없습니다. XML 은 중첩 깊이와 조각 수를 막습니다 — 트리는 글자보다 몇 배를 차지해서, 빈 태그만 든 작은 파일이 큰 메모리가 됩니다 — 트리는 재귀 없이 세우지만 그 뒤로 읽는 모든 코드가 재귀이고, Go 는 스택이 바닥나면 복구하지 못하고 죽습니다. 만들어진 문서 자체도 깊이와 조각 수를 확인한 뒤에 저장합니다. 링크는 다섯 형식 모두 `http`·`https`·`mailto` 만 남깁니다 — 파일이 `javascript:` 를 주소라고 우겨도 그것은 목적지가 아닙니다. 원격 주소는 어느 가져오기도 가져오지 않으므로, 파일이 내부망을 향하게 만들 수 없습니다.
