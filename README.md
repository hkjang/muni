...
| --- | --- | --- | --- |
| HWP | O | - | 한글 오피스의 옛 이진 형식입니다. OLE2 복합 파일 안에 압축된 이진 레코드가 들어 있습니다. Import 는 글자와 문단, 굵게·기울임·밑줄·취소선, 문단 정렬·들여쓰기, 제목 단계(개요 1~7), 표(가로·세로 병합), 그림(옛 파일의 BMP 는 PNG 로 바꿔서), 글상자와 묶음 개체 안의 글, 각주·미주를 읽습니다. 암호가 걸린 문서는 무엇이 문제인지 말하고 거절합니다. |
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·글자색·글꼴·크기, 표(가로·세로 병합, 머리글 행, 칸 음영·세로 정렬), 그림, 각주, 용지 방향을 양방향으로 처리합니다. 목록은 Export 에서 들여쓴 문단과 기호로 나갑니다. |
...
**두 한글 리더는 공개된 실제 파일로 확인했습니다** — pyhwp·hwp.js·rhwp 가 공개한 `.hwp` 열일곱 개와 `.hwpx` 열다섯 개(정부 결재 문서, 보도자료, 별지 서식 포함). 전부 읽히고, `.hwp` 열일곱 개는 `.hwpx` 로 써서 다시 읽어도 글자와 구조가 같습니다. `MUNI_CORPUS=<dir> go test ./internal/hwp/ ./internal/hwpx/ -run 'Corpus|RealRoundTrip' -v` 로 아무 폴더의 파일이든 같은 검사를 돌릴 수 있습니다.
