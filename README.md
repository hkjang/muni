...
| DOCX | O | O | 제목 단계, 글머리·번호·체크 목록(중첩 포함), 표(가로·세로 병합, 열 너비, 머리글 행, 칸 음영·세로 정렬), 굵게·기울임·밑줄·취소선·코드·형광·글자색·글꼴·크기·위첨자·아래첨자, 하이퍼링크, 문단 정렬·들여쓰기·줄간격, 인용, 코드 블록, 구분선, 쪽 나누기, 이미지(그려진 크기대로), 각주·미주, 목차, 머리글·바닥글, 용지 방향 |
| PDF | O | O | Import는 텍스트 레이어에서 제목·문단·목록·표·코드 블록·이미지(그려진 크기대로)를 복원합니다. Export는 Chromium 렌더링으로 화면과 같은 레이아웃을 만듭니다. |
...
**그림은 그려진 모양 그대로 다닙니다.** 문서가 그림을 담는 상자는 그림 바이트의 모양이 아닐 수 있어서 — 한 줄에 맞추어 눌러 놓은 직인이 그렇습니다 — 네 형식 모두 그린 너비와 높이를 함께 읽고, 편집기와 HTML·PDF 는 그 비율(`aspect-ratio`)로 그립니다. 화면과 인쇄의 스타일시트는 쪽보다 넓은 그림을 줄이려고 모든 그림에 `height:auto` 를 주는데, 그것만으로는 눌러 둔 그림이 바이트의 모양으로 되펴지기 때문입니다. 너비를 % 단추로 옮길 때는 높이도 같은 비율로 따라갑니다.
