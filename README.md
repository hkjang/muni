...
| --- | --- | --- | --- |
| HWP | O | - | 한글 오피스의 옛 이진 형식입니다. OLE2 복합 파일 안에 압축된 이진 레코드가 들어 있습니다. Import 는 글자와 문단, 굵게·기울임·밑줄·취소선, 문단 정렬·들여쓰기, 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 표(가로·세로 병합)와 표 캡션, 그림(옛 파일의 BMP 는 PNG 로 바꿔서), 글상자와 묶음 개체 안의 글, 각주·미주, 머리말·꼬리말을 읽습니다. 암호가 걸린 문서는 무엇이 문제인지 말하고 거절합니다. |
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·글자색·글꼴·크기, 표(가로·세로 병합, 머리글 행, 칸 음영·세로 정렬), 그림, 각주, 머리말·꼬리말, 용지 방향을 양방향으로 처리합니다. 쓰는 파일은 부품과 요소 하나하나가 한글 자신이 쓰는 모양입니다. |
