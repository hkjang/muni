...
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 표(가로·세로 병합, 머리글 행, 칸 음영·세로 정렬), 그림, 각주, 머리말·꼬리말, 용지 방향을 양방향으로 처리합니다. 쓰는 파일은 부품과 요소 하나하나가 한글 자신이 쓰는 모양입니다. |
