...
| PDF | O | O | Import는 텍스트 레이어에서 제목·문단·목록·표·코드 블록·이미지를 복원합니다. 쪽마다 되풀이되는 머리말·꼬리말과 쪽 번호는 종이의 것이지 문서의 것이 아니므로 버리고, 쪽을 넘어 이어지는 표는 하나로 잇습니다. 글자는 내장 글꼴의 자체 문자표로 확인합니다 — 부분 내장 글꼴에 딸린 `/ToUnicode` 는 글리프 번호를 그대로 유니코드라고 적어 두는 일이 잦아서, 그것만 믿으면 띄어쓰기가 `$` 나 제어 문자로 나옵니다. Export는 Chromium 렌더링으로 화면과 같은 레이아웃을 만듭니다. |
