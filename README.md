...
| HWPX | O | O | 한글 오피스가 쓰는 XML 형식입니다. 제목 단계(개요 1~7), 글머리표·번호 목록(중첩 포함), 문단 정렬·들여쓰기·줄간격, 굵게·기울임·밑줄·취소선·위첨자·아래첨자·글자색·글꼴·크기·글자 음영, 하이퍼링크, 표(가로·세로 병합, 열 너비, 머리글 행, 칸 음영·세로 정렬), 그림(그려진 크기대로), 각주, 머리말·꼬리말, 쪽 나누기, 인용문·코드 블록, 용지 방향을 양방향으로 처리합니다. 한글에는 인용문도 코드 블록도 없어서 muni 가 머리에 스타일 둘(`인용`/`Quote`, `코드`/`Code`)을 심어 두고 문단이 그것을 가리킵니다 — 들여쓰고 고정폭으로 그린 문단은 읽는 쪽에 그저 문단이라, 이름 말고는 되살릴 단서가 없습니다. 코드 블록은 한 문단 안에 줄바꿈으로 넣습니다. 줄마다 문단이면 끝을 알릴 것이 없어 나란한 코드 블록 둘이 하나로 돌아옵니다. 칸 음영은 칸이 아니라 머리의 `borderFill` 에 있어서, 칸만 보던 동안에는 한글이 쓴 어떤 파일에서도 읽히지 않았습니다. 쪽 나누기는 나뉜 자리가 아니라 *다음* 문단이 지니고 있어서 — `.hwp` 는 문단 나누기 종류의 셋째 비트, `.hwpx` 는 `<hp:p pageBreak="1">` — 그 문단 앞에 놓아야 합니다. 쓰는 파일은 부품과 요소 하나하나가 한글 자신이 쓰는 모양입니다. |
