...
화면은 muni v0.36.0 을 데모 데이터로 채워 실제로 찍은 것입니다(방문 추적 탭만 그 기능이 들어간
다음 빌드에서 찍었습니다).
...
| 방문 추적 | 어떤 화면이 쓰이는지 세는 스크립트를 페이지에 붙일지, 어느 수집기로 보낼지. 기본은 꺼짐입니다. [아래](#방문-추적) 에 따로 적었습니다 |
| 문서 넘기기 | 문서를 내려받지 않고 바로 보낼 수 있는 사내 서비스와, 문서를 받아 줄 서비스의 허용 목록. 기본은 비어 있습니다. [아래](#문서-넘기기) 에 따로 적었습니다 |
...
![서비스 설정 → 방문 추적 — Momento 수집기 설정과, 정책이 차단한 출처 목록의 「허용」 단추](assets/guide/admin-tracking.png)

어떤 화면이 실제로 쓰이는지 세려면 방문 추적 스크립트를 페이지에 붙입니다. **기본은 꺼짐**이라
새로 설치한 곳에서는 아무것도 달라지지 않고, 켜더라도 muni 가 보내는 것은 스크립트가 세는
쪽 번호와 경로뿐이며 문서 내용은 담기지 않습니다. 설정은 저장소에 두고 화면에서 바꾸므로
수집기 주소가 바뀌어도 다시 배포하지 않습니다.
...
| 측정 ID · Matomo 주소 · 사이트 id | GA4·GTM 과 Matomo 의 것. 구글 쪽은 폐쇄망에서 닿지 않습니다 |
...
**Momento 를 먼저 고르세요.** Momento 는 사내 자체 호스팅 수집기라 데이터가 밖으로 나가지 않는
유일한 선택지입니다. 「muni 를 거쳐 보내기」를 켜 두면(기본) 브라우저는 muni 하고만 이야기하고
muni 가 수집기로 넘기므로 아래의 보안 정책에 바깥 주소가 아예 등장하지 않습니다. 넘길 때
방문자의 세션 쿠키는 떼고 방문자 주소는 `X-Forwarded-For` 로 붙입니다. 프록시는 Momento 를
고르고 켜 둔 동안에만 답하며, 다른 어디로도 중계하지 않습니다.

#### 콘텐츠 보안 정책(CSP)과 왜 스니펫을 그냥 붙일 수 없는가

muni 의 모든 페이지는 `script-src 'self'` 로 잠긴 콘텐츠 보안 정책을 달고 나갑니다. 자기 오리진의
스크립트만 실행하므로 추적 코드를 그냥 끼워 넣으면 브라우저가 **조용히** 막고, 관리자는 수집기가
비어 있는 이유를 알 길이 없습니다. muni 는 그래서 다음을 함께 합니다.

1. **요청마다 nonce** — 페이지를 낼 때마다 새 nonce 를 만들어 스니펫의 모든 `<script>` 태그에 붙이고
   같은 값을 정책의 `script-src` 에 넣습니다. 정책을 `'unsafe-inline'` 으로 풀지 않습니다 — 한 번
   풀면 앱의 모든 인라인 스크립트가 함께 허용되고, 추적을 끈 뒤에도 느슨한 채 남기 때문입니다.
2. **출처는 코드에서 읽습니다** — 추적 도구는 자기 주소를 로더 안에 적어 둡니다. 붙여 넣은 코드에서
   `http(s)` 주소를 긁어 `script-src`·`connect-src`·`img-src` 에 더합니다. Momento(직접 연결)·Matomo 는
   적어 둔 주소, GA4·GTM 은 구글의 알려진 주소가 더해집니다.
3. **차단된 것을 기록합니다** — 추적이 켜진 동안에만 정책에 `report-uri /api/v1/tracking/csp-report`
   를 넣습니다. 브라우저가 막은 것을 신고하면 출처와 지시어를 메모리에 기억하고(서로 다른 출처
   100개까지, 데이터베이스에는 쓰지 않습니다) 이 탭의 「정책이 차단한 출처」에 보여 줍니다.
   「허용」을 누르면 그 출처 하나가 「추가로 허용할 출처」에 더해지고, 화면을 새로 고치면
   정책에 들어갑니다. 「목록 비우기」로 지운 뒤 다시 새로 고쳐 아직 막히는 것이 있는지 봅니다.

추적을 끄면 정책은 예전 그대로 좁아집니다. `/api/*`·`/mcp`·`/healthz`·`/readyz`·`/metrics`·`/momento/*`
같은 화면이 아닌 경로에는 스니펫이 붙지 않고, 정책은 오히려 더 좁은 `default-src 'none'` 입니다.
「서비스 관리 화면에서도 추적」이 꺼져 있으면 `/admin` 아래를 **직접 열었을 때** 스니펫이 빠지지만,
다른 화면에서 메뉴로 넘어온 경우에는 이미 실린 스크립트가 그대로 남습니다 — 화면 전환이
페이지를 다시 받지 않기 때문입니다.

**설정 순서**는 제공자와 주소를 넣고 「전체 저장」 → 아무 문서 화면을 새로 고침 → 수집기에서
방문이 들어오는지 확인 → 들어오지 않으면 이 탭의 차단 목록을 보고 「허용」 → 다시 새로 고침입니다.

### 문서 넘기기

생각은 umm 의 캔버스에서 시작해 muni 의 문서가 되고 ptium 의 슬라이드가 되어 weekly 의 보고로
들어갑니다. 단계마다 사람이 파일을 내려받아 다시 올리지 않도록, muni 는 사내 서비스 표준
(HANDOFF-STANDARD)대로 **다른 서비스로 보내기**와 **다른 서비스에서 받기**를 둘 다 합니다.
muni 는 `markdown` 과 `docx` 를 보내고 `markdown` 을 받습니다.

서비스끼리 서로의 자격 증명을 들고 있지 않습니다. 보내는 쪽이 **5분 동안 한 번만 쓸 수 있는
표(claim)** 를 발급하고, 받는 쪽이 표를 들고 원본에서 직접 받아 갑니다. 표는 그 사람이 읽을 수
있는 그 문서 하나에만 묶이고, 해시로만 저장되며, 요청 로그와 감사 로그 어디에도 적히지 않습니다.

**기본은 비어 있습니다.** 허용 목록이 비어 있는 동안에는 문서 화면의 내보내기 메뉴에 「…으로
보내기」가 나타나지 않고, 다른 서비스가 보낸 문서도 받지 않습니다 — 새로 설치한 곳에서는
아무것도 달라지지 않습니다.

| 설정 | 뜻 |
| --- | --- |
| 주소 (오리진) | 상대 서비스의 `https://호스트` 까지. 경로·쿼리 없이 적습니다. 대소문자는 가리지 않고, 포트가 다르면 다른 서비스입니다 |
| 이름 | 메뉴에 보일 이름. 비우면 호스트가 보입니다 |
| 그 서비스가 받는 형식 | 표준의 형식 표에서 그 서비스의 「받는다」 칸. muni 가 보내는 `Markdown`·`DOCX` 중 그 서비스가 받는 것만 체크합니다. 둘 다 아니면 보낼 곳으로 나오지 않습니다 |

같은 목록이 양쪽에 다 쓰입니다 — 여기 적은 서비스로만 보내고, 여기 적은 주소에서만 받습니다.
상대 서비스의 허용 목록에도 이 muni 의 주소(브라우저가 muni 에 닿는 그 주소, `X-Forwarded-Proto`
를 지킵니다)를 같은 방식으로 적어야 양쪽이 이어집니다. 표준의 표대로면 ptium 과 weekly 는
`Markdown`·`DOCX` 를, muni 는 `Markdown` 을 받고, kanpic 과 umm 은 muni 가 보내는 형식을 받지
않으므로 적어도 보낼 곳으로 나오지 않습니다. `DOCX` 는 「보안·내보내기」의 DOCX 내보내기가
켜져 있을 때만 보냅니다.

**받는 쪽이 지키는 것.** `/handoff?source=…&claim=…` 로 들어온 `source` 는 밖에서 온 값입니다.
그대로 받아 오면 muni 가 사내 아무 주소나 대신 긁어 오는 도구가 되므로:

- 허용 목록에 없는 `source` 는 **아무것도 요청하지 않고** 거절합니다. 호스트가 같아도 경로가
  붙어 있으면 오리진이 아니므로 거절합니다.
- 리다이렉트는 따라가지 않습니다. 상대가 다른 곳으로 넘기려 하면 그 자리에서 멈춥니다.
- 받는 크기는 **25MB**, 시간은 **30초**가 상한이고, 넘으면 중간에 끊습니다.
- 응답의 `Content-Type` 이 `text/markdown` 이 아니면 본문을 읽지 않고 버립니다.
- 받은 문서는 그 사람의 **개인 워크스페이스**에 만들어지고, 첫 버전의 사유에 `handoff:<출처>`
  가, 감사 로그에 `HANDOFF_RECEIVE` 와 출처·파일 이름이 남습니다. 어느 캔버스에서 온 문서인지
  나중에 물어볼 수 있습니다.

실패하면 JSON 이 아니라 사람이 읽을 화면이 나옵니다 — 허용되지 않은 서비스(403), 표가
만료됐거나 이미 쓰임(404), 읽을 수 없는 형식(415), 너무 큼(413), 상대가 응답하지 않음(502).
로그인이 없으면 로그인 화면으로 보냈다가 같은 주소로 돌아옵니다.

**보내는 쪽.** 문서 화면의 내보내기 메뉴에서 「<이름> 으로 보내기 (Markdown)」을 고르면 새
탭에서 상대 서비스의 `/handoff` 가 열리고, 상대가 표를 들고 muni 의
`GET /api/v1/handoff/claims/{claim}` 에서 문서를 받아 갑니다. 표를 만든 순간의 문서가 나가며,
받아 가는 순간 표는 사라집니다. 감사 로그에는 `HANDOFF_ISSUE`(발급)와 `HANDOFF_SERVE`(내줌)가
문서 ID 와 형식으로 남고, 표 자체는 남지 않습니다.
...
| 「…으로 보내기」가 메뉴에 없다 | 「문서 넘기기」 탭의 목록과 체크한 형식 | 목록이 비어 있거나 그 서비스가 `Markdown`·`DOCX` 를 받지 않는 것으로 적혀 있으면 나오지 않습니다. `DOCX` 는 DOCX 내보내기도 켜져 있어야 합니다([문서 넘기기](#문서-넘기기)) |
| 다른 서비스가 보낸 문서가 「허용되지 않은 서비스」로 거절된다 | 「문서 넘기기」 탭의 주소 | 상대가 `source` 로 보내는 오리진이 목록의 주소와 스킴·호스트·포트까지 같아야 합니다. 경로가 붙은 주소는 오리진이 아닙니다 |
...
- 모든 페이지는 `script-src 'self'` 의 콘텐츠 보안 정책을 답니다. 방문 추적을 켜도 요청마다 다른
  nonce 와 스니펫이 적은 출처만 더해지고 `'unsafe-inline'` 은 넣지 않습니다([방문 추적](#방문-추적)).
