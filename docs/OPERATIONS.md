...
### 알아두실 것: 공개 범위 `LINK`는 아직 아무것도 하지 않습니다

문서 공개 범위에 `LINK` 값이 있고 API가 받아들이지만, **링크로 문서를 여는 경로가 muni에 아직 없습니다.** 접근 판정에도 들어 있지 않습니다. 즉 `LINK`로 설정된 문서는 실제로는 「제한됨」과 똑같이 동작합니다 — 새어 나가지는 않지만, 설정한 사람은 무언가를 켰다고 믿게 됩니다.

관리자 설정의 「공개 링크 공유 허용」도 마찬가지로 지금은 아무 효과가 없습니다. 권한 보기 화면은 이런 문서를 만나면 그 사실을 그대로 알려 줍니다.
