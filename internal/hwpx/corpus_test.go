...
func TestCorpus(t *testing.T) {
	...
	files, _ := filepath.Glob(filepath.Join(dir, "*.hwpx"))
	sort.Strings(files)
	for _, path := range files {
		...
		t.Logf("%-38s land=%v blocks=%d p=%d h=%d tbl=%d img=%d fn=%d assets=%d | %s",
			name, meta.Landscape, len(document.Content), counts["paragraph"], counts["heading"],
