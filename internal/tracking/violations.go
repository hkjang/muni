...
type Violation struct {
	...
	// Allowed marks an origin the current configuration already lets
	// through, so a fixed snippet stops nagging without anybody clearing it.
	Allowed bool `json:"allowed"`
}

// Recorder collects the policy violations browsers report. It is in memory on
// purpose: the reports are a live troubleshooting aid for the person pasting a
// snippet, not an audit record, and keeping them out of the database means a
...
// allowing it is neither possible nor wanted.
...
func (r *Recorder) List(config Config) []Violation {
	...
	for _, violation := range r.violations {
		copied := *violation
		copied.Allowed = allowed[copied.Origin] || matchesWildcard(copied.Origin, allowed)
...
// Forget drops the recorded reports, which is what an administrator does
